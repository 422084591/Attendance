...
<script type="application/x-javascript">
	 ...
	 function submit_confirm(){
		 var salariedMan = document.getElementById("salariedMan").value;
						...
						<div id="main">
							...
							<s:if test="paySalary.id!=0">
								<div class="kv-item">
									...
									<input readonly="readonly" type="text" name="id" class="text-align" id="ID" placeholder="*必填项" autofocus size="25" title="ID" value='<s:property value="PaySalary.id"/>'>
								</div>
							</s:if>

									<div class="kv-item">
								<span class="kv-label"><label class="tsl"
									for="salariedMan">领薪人：</label> </span> 
									<select name="salariedMan">
							...
							<div class="kv-item">
								<span class="kv-label"><span style="color: red">*</span>
									<label class="tsl" for="{领薪时间}">领薪时间：</label> </span> 
									<input type="text" name="paidTime" class="text-align" id="time" placeholder="请输入派薪时间" autofocus size="25" title="领薪时间" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value='<s:property value="PaySalary.paidTime"/>'>
							</div>
							<div class="kv-item">
								<span class="kv-label"> <label class="tsl" for="time">领薪区间：</label>
								</span> 
								<input type="text" name="calculateStartDate" class="text-align" id="calculateStartDate" placeholder="请输入开始日期" autofocus size="25" title="开始日期" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'calculateEndDate\')}'})" value='<s:property value="paySalary.calculateStartDate"/>'>
								<br> 
								<input type="text" name="calculateEndDate" class="text-align" id="calculateEndDate" placeholder="请输入结束日期" autofocus size="25" title="结束日期" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'calculateStartDate\')}'})" value='<s:property value="PaySalary.calculateEndDate"/>'>
							</div>
							<div class="kv-item">
