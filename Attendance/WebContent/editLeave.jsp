...
function leave(){
	...
	if((yearNumber%4==0&&yearNumber%100!=0)||yearNumber%1000==0){
		if(monthNumber==2){
			if(newDay>29){newDay-=29;monthNumber+=1;}
		}else{
			if(newDay>28){
				newDa-=28;monthNumber+=1;}}
		}
	if(monthNumber==4||monthNumber==6||monthNumber==9||monthNumber==11){
		if(newDay>30){
			newDay-=30;monthNumber+=1;
		}
	}
	if(monthNumber==1||monthNumber==3||monthNumber==5||monthNumber==7||monthNumber==8||monthNumber==10){
		if(newDay>31){
			newDay-=31;monthNumber+1;
		}
	}
	if(monthNumber==12){
		if(newDay>31){
			newDay-=31;monthNumber=1;yearNumber+=1;
		}
	}
	var y=String(yearNumber);
	if(monthNumber<10){var mo='0'+String(monthNumber);}else{var mo=String(monthNumber);}
	if(newDay<10){var d='0'+String(newDay);}else{var d=String(newDay);}
	if(newHour<10){var h='0'+String(newHour);}else{var h=String(newHour);}
	if(newMinute<10){var mi='0'+String(newMinute);}else{var mi=String(newMinute);}
			...
			<div id="sidebar" class="nav-collapse">
                ...
                <li class="sub-menu">
                    ...
                    <ul class="sub">
                        ...
                        <li><a class="active" href="Leave_limit"> 请假单</a></li>
                    </ul>
                </li>
                <li class="sub-menu">
                    <a href="javascript:;">
							...
							<s:else>
								<form name="personalForm" method="post"
									...
									<span class="kv-label"> <label class="tsl" for="ID">ID：</label>
										...
										size="25" title="ID"
										value='<s:property value="leave.id"/>'>
								</div>
							</s:else>
							<div class="kv-item">
								<span class="kv-label"><span style="color:red">*</span> <label class="tsl" for="pleaseDummyMan">请假人：</label>
								</span> <input type="text" name="pleaseDummyMan" class="text-align"
									id="pleaseDummyMan" placeholder="请输入请假人" autofocus size="25" title="请假人"
									value="${sessionScope.User.username}">
							</div>
							<div class="kv-item">
								<span class="kv-label"> <label class="tsl" for="time">请假时间：</label>
								</span>
								<input type="text" name="startTime" class="text-align"
									id="startTime" placeholder="请输入开始日期" autofocus size="25" title="开始日"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',maxDate:'#F{$dp.$D(\'endTime\',{m:-30})}'})">
<br>
<label>↓请在下方选择请假时长↓</label>
<br>
<label>分钟:</label>
...
<select id="day" name="day">
	...
	<option value="30">30</option>
				...
				<input type="text" name="endTime" id="endTime" readonly="readonly" placeholder="请选择请假时长" autofocus size="25" title="截止日"  onClick="leave()">
							</div>
							<div class="kv-item">
								<span class="kv-label"> <label class="tsl" for="leaveReason">请假原因：</label>
								</span> <input type="text" name="leaveReason" class="text-align"
									id="leaveReason" placeholder="请输入请假原因" autofocus size="25" style="height: 100px;" title="备注"
